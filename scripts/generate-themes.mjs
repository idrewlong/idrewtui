...
/** Opaque blend of `a` toward `b` by `t` (0-1). Always solid — never a translucent color-mix. */
const blendHex = (a, b, t) => toHex(rgb(a).map((c, i) => c + (rgb(b)[i] - c) * t))
...
export function generateThemes() {
  ...
  for (const name of THEME_ORDER) {
    ...
    // Opaque highlight background for hovered/selected rows — a step from
    // `surface` toward `line`, computed once here so it never depends on
    // whatever happens to sit behind it (that dependency is what caused the
    // contrast regression a translucent color-mix() produced).
    const surfaceHi = blendHex(toHex(rgb(surface)), toHex(rgb(line)), 0.4)
    const bases = [bg, surface, surfaceHi]
...
function toCss(themes) {
  ...
  color-scheme: ${t.mode};
${Object.entries(t.tokens).map(([k, v]) => `  --${kebab(k)}: ${v};`).join('\n')}
  --theme-name: "${t.name}";
  --theme-mode: "${t.mode}";
}`
  ...
  const defaults = t => Object.entries(t.tokens).map(([k, v]) => `  --${kebab(k)}: ${v};`).join('\n')
